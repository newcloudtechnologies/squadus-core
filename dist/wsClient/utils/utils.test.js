...
describe('wsClient utils', () => {
    describe('isLoginPass', () => {
        it('should return true on correct data', () => {
            ...
            expect((0, utils_1.isLoginPass)(correctLoginPass)).toBeTruthy();
        });
        it('should return false on empty strings', () => {
            ...
            expect((0, utils_1.isLoginPass)(wrongLoginPass)).toBeFalsy();
        });
    });
    describe('isLoginOAuth', () => {
        it('should return true on correct data', () => {
            ...
            expect((0, utils_1.isLoginOAuth)(correctCredentialsOAuth)).toBeTruthy();
        });
        it('should return false on empty strings', () => {
            ...
            expect((0, utils_1.isLoginOAuth)(wrongCredentialsOAuth)).toBeFalsy();
        });
    });
    describe('isLoginAuthenticated', () => {
        it('should return true on correct data', () => {
            ...
            expect((0, utils_1.isLoginAuthenticated)(correctCredentialsAuthenticated)).toBeTruthy();
        });
        it('should return false on empty strings', () => {
            const wrongCredentialsAuthenticated = {
                resume: '',
            };
            expect((0, utils_1.isLoginAuthenticated)(wrongCredentialsAuthenticated)).toBeFalsy();
        });
    });
    describe('isLoginResult', () => {
        it('should return true on correct data', () => {
            ...
            expect((0, utils_1.isLoginResult)(correctCredentialsAuthenticated)).toBeTruthy();
        });
        it('should return false on empty strings', () => {
            const wrongCredentialsAuthenticated = {
                token: '',
            ...
            // @ts-expect-error wrong type on purpose
            expect((0, utils_1.isLoginResult)(wrongCredentialsAuthenticated)).toBeFalsy();
        });
    });
    describe('getReopenInterval', () => {
        const counter = constants_1.MAX_WS_RECONNECT_INTERVAL / constants_1.INITIAL_RECONNECT_INTERVAL;
        it('should return MAX INTERVAL if counter is height ', () => {
            expect((0, utils_1.getReopenInterval)(counter)).toBe(constants_1.MAX_WS_RECONNECT_INTERVAL);
        });
        it('should return correct interval', () => {
            const interval = constants_1.INITIAL_RECONNECT_INTERVAL * (counter - 1);
            expect((0, utils_1.getReopenInterval)(counter - 1)).toBe(interval);
        });
    });
    describe('hostToWS', () => {
        it('should return ws host with ssl', () => {
            expect((0, utils_1.hostToWS)('https://im.example.ru')).toBe('wss://im.example.ru');
        });
        it('should return ws host without ssl', () => {
            expect((0, utils_1.hostToWS)('http://im.example.ru')).toBe('ws://im.example.ru');
        });
    });
    describe('isSubscriptionData', () => {
        it('should return true only with subscriptionData ', () => {
            ...
            // @ts-expect-error wrong type on purpose
            (0, utils_1.isSubscriptionData)(object));
    ...
    describe('isAuthorizedData', () => {
        it('should return true only with authorizedData', () => {
            const filterResult = Object.entries(wsEvents).filter(([, object]) => 
            // @ts-expect-error wrong type on purpose
            (0, utils_1.isAuthorizedData)(object));
            expect(filterResult.length).toBe(1);
    ...
    describe('isCallResponse', () => {
        it('should return true only with callResponse', () => {
            const filterResult = Object.entries(wsEvents).filter(([, object]) => 
            // @ts-expect-error wrong type on purpose
            (0, utils_1.isCallResponse)(object));
            expect(filterResult.length).toBe(1);
    ...
    describe('isSubscriptionResponse', () => {
        it('should return true only with subscriptionResponse', () => {
            const filterResult = Object.entries(wsEvents).filter(([, object]) => 
            // @ts-expect-error wrong type on purpose
            (0, utils_1.isSubscriptionResponse)(object));
            expect(filterResult.length).toBe(1);
    ...
    describe('isUnsubscribeResponse', () => {
        it('should return true only with unsubscribeResponse', () => {
            const filterResult = Object.entries(wsEvents).filter(([, object]) => 
            // @ts-expect-error wrong type on purpose
            (0, utils_1.isUnsubscribeResponse)(object));
            expect(filterResult.length).toBe(1);
    ...
    describe('getValidHost', () => {
        ...
        it('should return modified host address', () => {
            expect((0, utils_1.getValidHost)(host)).toBe(`ws://${host}/websocket`);
        });
        it('should return modified host address with additional S in prefix', () => {
            expect((0, utils_1.getValidHost)(hostWithSsl)).toBe(`wss://${host}/websocket`);
        });
    });
    describe('loginParams', () => {
        it('should return source login credentials', () => {
            ...
            expect((0, utils_1.loginParams)(correctLoginPass)).toBe(correctLoginPass);
        });
        it('should return source OAuth credentials', () => {
            ...
            expect((0, utils_1.loginParams)(correctCredentialsOAuth)).toBe(correctCredentialsOAuth);
        });
        it('should return source authenticated credentials', () => {
            const correctCredentialsAuthenticated = {
                resume: 'token',
            };
            expect((0, utils_1.loginParams)(correctCredentialsAuthenticated)).toBe(correctCredentialsAuthenticated);
        });
        it('should rename "token" key to "resume" key', () => {
            ...
            // @ts-expect-error wrong type on purpose
            expect((0, utils_1.loginParams)(sourceCredentialsAuthenticated)).toStrictEqual(correctCredentialsAuthenticated);
        });
        it('should return correctly reformated credentials', () => {
            ...
            expect((0, utils_1.loginParams)(credentials)).toStrictEqual(responseCredentials);
        });
    });
});
